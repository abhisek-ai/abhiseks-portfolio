# Abhisek's Portfolio

A high-tech, production-grade personal portfolio built for an AI/ML Engineer. Designed with a custom **"Neural Nexus"** theme featuring a deep black aesthetic, neon cyan accents, and technical typography.

**Live Site:** [https://abhisek-ai.github.io/abhiseks-portfolio](https://abhisek-ai.github.io/abhiseks-portfolio)

![Portfolio Preview](src/assets/photo.jpg) 
*(Note: Replace this with a screenshot of your actual website later)*

## Features

*   **Responsive Design**: Fully responsive layout optimized for all devices.
*   **Custom Theme**: "Neural Nexus" design system with `Space Grotesk` and `JetBrains Mono` fonts.
*   **Dynamic Components**:
    *   **Hero**: Typed role effect and detailed bio.
    *   **Experience**: Vertical timeline of professional history.
    *   **Projects**: Grid layout highlighting tech stacks.
    *   **Publications**: showcases research papers.
    *   **Contact**: Functional form integrated with Formspree.
*   **Performance**: Built with Vite for instant server start and lightning-fast HMR.

## Tech Stack

*   **Frontend**: React 19, JavaScript (ES6+)
*   **Build Tool**: Vite
*   **Styling**: Vanilla CSS3 (Variables, Flexbox, Grid)
*   **Deployment**: GitHub Pages

## Getting Started

### Prerequisites

*   Node.js (v18 or higher)
*   npm

### Installation

1.  Clone the repository:
    ```bash
    git clone https://github.com/abhisek-ai/abhiseks-portfolio.git
    cd abhiseks-portfolio
    ```

2.  Install dependencies:
    ```bash
    npm install
    ```

3.  Start the development server:
    ```bash
    npm run dev
    ```

## Deployment

This project is configured to deploy to GitHub Pages.

1.  Push your changes to the `main` branch.
2.  Run the deploy script:
    ```bash
    npm run deploy
    ```

## 📄 License

This project is licensed under the MIT License - see the LICENSE file for details.

---
**Abhisek Mallick** | MS DAE @ Northeastern | AI Researcher & Engineer
